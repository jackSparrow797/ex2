...
if ($this->startResultCache(false, array($arNavigation))) 
{
	...
	$arResult["CLASSIFIER_ID"] = array();
	$rsElement_link = CIBlockElement::GetList(false, $arFilterСlassifier, false, array("nPageSize" => 1), $arSelectСlassifier);
	...
	$arFilterElems = array (
		...
		"ACTIVE_DATE" => "Y",
		"CHECK_PERMISSIONS" => "Y",
		"PROPERTY_LINK_ELEMENT" => $arResult["CLASSIFIER_ID"]
	);
;
	...
	{
		//dump($arElement);
		$arElement["CLASSIFIER"] = $arElement["PROPERTY_LINK_ELEMENT_VALUE"];
		/*if($arElement["CLASSIFIER"] == "")
		{
			$arElement["CLASSIFIER"] = "EMPTY";
		}*/
		$arResult["ELEMENTS"][] = $arElement;
		$arResult["COUNT_CLASS"]++;
	}
...
$APPLICATION->SetTitle("Каталог Продукция. Элементов  - ".$arResult["COUNT_CLASS"]);