...
if ($this->startResultCache(false, false)) 
{
	...
	$arResult["SECTION_CLASSIFIER"] = array();
	...
	while($arElement = $rsElementСlassifier->GetNext())
	{
		...
		{
			$arResult["SECTION_CLASSIFIER"][$elem][] = $arElement["ID"];
		}
	}
	$arResult["COUNT_CLASSIFIER"] = count($arResult["CLASSIFIER"]);
	

	//в этом варианте связи можно и невытаскивать разделы, а не, придетая все равно название же :(
	//Вытаскиваем разделы
	...
	{
		//строим связи классификатор - разделы
	...
	{	
		...
		foreach($arResult["SECTION_CLASSIFIER"][$arElement["IBLOCK_SECTION_ID"]] as $elem)
		{
